...
/*   Updated: 2026/03/12 15:52:28 by thlibers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	exec->heredoc.heredoc_fd[exec->heredoc.heredoc_fd_size] = open(HEREDOC_F,
			O_WRONLY | O_CREAT | O_TRUNC, 00644);
	if (exec->heredoc.heredoc_fd[exec->heredoc.heredoc_fd_size] < 0)
	{
...
int	terminate_heredoc(t_exec *exec)
{
	...
	exec->heredoc.heredoc_fd[exec->heredoc.heredoc_fd_size] = open(HEREDOC_F, O_RDONLY);
	if (exec->heredoc.heredoc_fd[exec->heredoc.heredoc_fd_size] < 0)
	{
		ft_fprintf(STDERR_FILENO, "Failed to reopen heredoc file for reading");
		return (0);
	}
	exec->heredoc.heredoc_fd_size++;
	unlink(HEREDOC_F);
	return (1);
}
