...
{
	exec->heredoc.hd_fd[exec->heredoc.hd_fd_size] = open(HEREDOC_F,
			O_WRONLY | O_CREAT | O_TRUNC, 00644);
	if (exec->heredoc.hd_fd[exec->heredoc.hd_fd_size] < 0)
	{
...
int	terminate_heredoc(t_exec *exec)
{
	...
	exec->heredoc.hd_fd[exec->heredoc.hd_fd_size] = open(HEREDOC_F, O_RDONLY);
	if (exec->heredoc.hd_fd[exec->heredoc.hd_fd_size] < 0)
	{
		ft_fprintf(STDERR_FILENO, "Failed to reopen heredoc file for reading");
		return (0);
	}
	exec->heredoc.hd_fd_size++;
	unlink(HEREDOC_F);
	return (1);
}
